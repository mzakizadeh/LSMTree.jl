...
function buffer_dump(store::AbstractStore{K, V, PAGE, <:Any}) where {K, V, PAGE}
    ...
    current = merge(store, first_level, entries[], indices, islast(first_level))
    free_page(page)
...
function compact(store::AbstractStore{K, V}) where {K, V}
    ...
    current = first_level
    _current = current !== nothing ? current[] : nothing
    next = current !== nothing ? get_level(_current.next_level, store) : nothing
    _next = next !== nothing ? next[] : nothing

    force_remove = false
    
    while next !== nothing
        if !isfull(_next)
            force_remove = islast(_next)
            break
        end
    ...
    if next === nothing || isfull(_next)
        ...
        new_level = malloc_and_init(Level{K, V},
                                    ...
                                    data.table_threshold_size)
        store.meta.levels_bf[new_id] = BloomFilter(new_size, 0.001)
        new_level.prev_level[] = _last_level.id
        last_level.next_level[] = new_id
    ...
    after_next = get_level(_next.next_level, store)
    _after_next = after_next !== nothing ? after_next[] : nothing
    while !isfirst(_next)
        ...
        if after_next !== nothing 
            next.next_level[] = _after_next.id
            _next = next[]
            after_next.prev_level[] = _next.id
            set_level(after_next, store)
        end
        current_isfirst = isfirst(current[])
        current = empty(current[], store)
        current.next_level[] = _next.id
        _current = current[]
        next.prev_level[] = _current.id
        if current_isfirst store.data.first_level[] = _current.id end
        set_level(next, store)
        set_level(current, store)
        after_next = next
        _after_next = _next
        next = current
        _next = _current
        current = get_level(_current.prev_level, store)
        force_remove = false
    end
end
